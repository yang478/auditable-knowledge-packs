...
- `scripts/kbtool.py` — deterministic `search`/`bundle`/`reindex` commands to turn a query into `bundle.md` (with a reference list back into `references/`)
...
- **Plain keyword search** (grep/Elasticsearch/SQLite FTS) is good for finding strings, but you still need a deterministic, structure-aware context assembler; `kbtool.py bundle` adds rerank + expansion + budgeted rendering into a single evidence file.
...
query     ──(kbtool.py bundle)─> bundle.md  (evidence + citations)
```
...
pack-builder/
  ...
  templates/                    # generated skill templates (kbtool.py, reindex.py)
docs/
