...
核心思想：**不要让 LLM 承担检索拼装**。相反，通过脚本生成可审计的证据包（`bundle.roundNN.md`），并强制携带来源信息，同时输出机器可读的 trace/verify 产物。
...
kbtool.py research（在线查询，单轮）
  ├─ （可选）hook：pre_search
  ├─ search：确定性迭代检索（≤5 轮，尽量收敛到 2–3 条款/父节点）
  ├─ rerank：确定性打分与融合
  ├─ （可选）hook：post_search / pre_expand
  ├─ expand：邻接扩展 / 父链导航 / 受控的一跳补查
  ├─ render：按预算渲染 bundle.roundNN.md + 来源清单 + 检索轨迹 + 回答约束
  └─ verify + trace：写出 trace.roundNN.json / verify.roundNN.json，并追加 trace.jsonl
...
2) **运行时 Hooks（显式启用）**：`kbtool.py search|research --enable-hooks` 会在若干阶段尝试执行 `hooks/*.py`（如 `pre_search`/`post_search`/`pre_expand`/`pre_render`），用于局部改写查询、过滤候选、控制扩展或脱敏渲染。默认关闭以保持确定性与可审计的基线行为。

## 关键设计原则

- **结构优先**：尽量保留章/节/条等结构，便于人工抽查与导航。
- **来源强制**：bundle 中每段内容都保留可回跳到 `references/` 的信息。
- **脚本确定性**：`research`（单轮）目标是跨模型、跨提示词依然可重复。
- **可审计轨迹**：`bundle.roundNN.md` 会包含 `## 检索轨迹`；同时 `trace.roundNN.json` / `verify.roundNN.json` 提供机器可读的审计信息。
