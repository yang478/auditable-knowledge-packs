    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f".{skill_name}.tmp-", dir=out_dir) as tmp_name:
        ...
        if ir_jsonl is not None:
            ...
            for p in inputs:
                md = extract_to_markdown(p, pdf_fallback=pdf_fallback)
        ...
        write_text(
            tmp / "SKILL.md",
            render_generated_skill_md(skill_name, title, docs, category_overrides=category_overrides),
        )
        ...
        write_kb_sqlite_db(tmp / "kb.sqlite", merged_docs, merged_nodes, merged_edges, merged_aliases, base_dir=tmp)
