...
def build_match_expression(
    ...
    *,
    ...
    for t in must_terms:
        clause = build_match_all(tokenize_cjk_2gram(t) + [m.group(0).lower() for m in _ASCII_WORD_RE.finditer(t)], max_tokens=16)
        if clause:
    ...
    if query_mode == "and":
        ...
        q_clauses: List[str] = []
        for p in parts:
            clause = build_match_all(tokenize_cjk_2gram(p) + [m.group(0).lower() for m in _ASCII_WORD_RE.finditer(p)], max_tokens=16)
            if not clause:
                continue
            q_clauses.append(f"({clause})" if " AND " in clause else clause)
        query_clause = " AND ".join(q_clauses)
    else:
        query_clause = build_match_query(tokenize_cjk_2gram(raw_query) + [m.group(0).lower() for m in _ASCII_WORD_RE.finditer(raw_query)], max_tokens=max_tokens)
